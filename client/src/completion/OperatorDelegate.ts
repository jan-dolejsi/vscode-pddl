    ...
    getOperatorItems(): CompletionItem[] {
        if (!this.operatorItems) {
            ...
            this.operatorItems = [
                ...
                this.createOperator('forall', 'For all condition', new MarkdownString('Conjunction condition for all objects of specified type. For example:').appendCodeblock('(forall (?p - product) (available ?p))', PDDL)),
