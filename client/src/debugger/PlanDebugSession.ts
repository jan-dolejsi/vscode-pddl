...
export class PlanDebugSession extends LoggingDebugSession {

	// we don't support multiple threads, so we can use a hardcoded ID for the default thread
	private static THREAD_ID = 1;

	// a plan debug runtime (or debugger)
	private _runtime: PlanDebugRuntime;
	...
	public constructor() {
		...
		this._runtime = new PlanDebugRuntime();

		// setup event handlers
		this._runtime.on('stopOnEntry', () => {
			this.sendEvent(new StoppedEvent('entry', PlanDebugSession.THREAD_ID));
		});
		this._runtime.on('stopOnStep', () => {
			this.sendEvent(new StoppedEvent('step', PlanDebugSession.THREAD_ID));
		});
		this._runtime.on('stopOnBreakpoint', () => {
			this.sendEvent(new StoppedEvent('breakpoint', PlanDebugSession.THREAD_ID));
		});
		this._runtime.on('stopOnException', () => {
			this.sendEvent(new StoppedEvent('exception', PlanDebugSession.THREAD_ID));
		});
		this._runtime.on('breakpointValidated', (bp: HappeningBreakpoint) => {
	...
	protected threadsRequest(response: DebugProtocol.ThreadsResponse): void {
		...
		response.body = {
			threads: [
				new Thread(PlanDebugSession.THREAD_ID, "thread 1")
			]
		};
		this.sendResponse(response);
	}
	...
	private createSource(filePath: string): Source {
		return new Source(basename(filePath), this.convertDebuggerPathToClient(filePath), undefined, undefined, 'plan-debug-adapter-data');
	}
}
