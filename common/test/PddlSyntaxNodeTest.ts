    ...
    */
    ...
    describe('#getText()', () => {
        ...
        it('gets type declaration node text', () => {
            // GIVEN
            let originalPddl = `child1 child2 - parent`;
            let tree = new PddlSyntaxTreeBuilder(originalPddl).getTree();
    ...
    describe('#getNonCommentText()', () => {
        it('should return the same when no comments are present', () => {
            // GIVEN
            let originalPddl = `:precondition (and (p))`;
            let tree = new PddlSyntaxTreeBuilder(originalPddl).getTree();
        ...
        it('strips comments', () => {
            // GIVEN
            let originalPrefixPddl = `:precondition (and `;
            let comment = `; (p)`;
    ...
    describe('#getParameterDefinition()', () => {
        it('finds parameter definition in (:action', () => {
            ...
            let parameter = parameterDefinitionNode && parseParameters(parameterDefinitionNode.getText()).find(p => p.name === parameterName);
